...
class NVDiffRenderer(torch.nn.Module):
    def __init__(
        ...
        self.glctx = dr.RasterizeCudaContext()    # lxy 20240404
    ...
    def render_mesh(
        ...
        # rast_out, rast_out_db = dr.rasterize(self.glctx, verts_clip, tri, image_size)  # *** RuntimeError: width and height must be divisible by 8  
        # lxy:
        try:
            rast_out, rast_out_db = dr.rasterize(self.glctx, verts_clip, tri, image_size)   
        except:
            image_size_8 = (image_size[0] // 8 * 8, image_size[1] // 8 * 8)
            rast_out_8, rast_out_db = dr.rasterize(self.glctx, verts_clip, tri, image_size_8)
            rast_out = torch.nn.functional.interpolate(rast_out_8.permute(0,3,1,2), size=(image_size[0], image_size[1]), mode='bilinear', align_corners=False).permute(0,2,3,1)
        ...
        return {
            ...
            'rgba': rgba_aa.flip(1),
            'verts_camera': verts_camera,    # lxy # 20240505
        }
