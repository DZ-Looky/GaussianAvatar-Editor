...
def readMeshesFromTransforms(path, transformsfile):
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in tqdm(enumerate(frames), total=len(frames)):
            ...
            # flame_param.keys()   ['translation', 'rotation', 'neck_pose', 'jaw_pose', 'eyes_pose', 'shape', 'expr', 'static_offset']
            #                      [1,3]              [1, 3]     [1, 3]     [1, 3]        [1, 6]      [300, ]  [1,100]    [1, 5143, 3]
    return mesh_infos

def readDynamicNerfInfo(path, white_background, eval, extension=".png", target_path="", render_cano=False):  # in     
    ...
    if render_cano:   # lxy
        print("Reading Canonical Transforms")
        # cano_cam_infos = readCamerasFromTransforms(path, "transforms_cano.json", white_background, extension)
        cano_cam_infos = readCamerasFromTransforms(path, "transforms_cano_interploate.json", white_background, extension)
        # cano_cam_infos = readCamerasFromTransforms(path, "transforms_cano_interploate_faceCam.json", white_background, extension)   # 渲染不出东西
        # cano_cam_infos = readCamerasFromTransforms(path, "transforms_cano_interploate_faangzhouCam.json", white_background, extension)   # 渲染不出东西
    ...
    # camera_16 = readCamerasFromTransforms(path, "transforms_16_cam.json", white_background, extension)
