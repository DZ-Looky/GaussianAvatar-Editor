...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : Union[GaussianModel, FlameGaussianModel], load_iteration=None, shuffle=True, render_cano=False, resolution_scales=[1.0]):
        """b
        ...
        elif os.path.exists(os.path.join(args.source_path, "canonical_flame_param.npz")):  # in
            print("Found FLAME parameter, assuming dynamic NeRF data set!")
            scene_info = sceneLoadTypeCallbacks["DynamicNerf"](args.source_path, args.white_background, args.eval, target_path=args.target_path, render_cano=render_cano)
        ...
        if not self.loaded_iter:  # when render, not in this
        ...
        for resolution_scale in resolution_scales:
            ...
            print("Loading Canonical Cameras")
            self.cano_cameras[resolution_scale] = cameraList_from_camInfos(scene_info.cano_cameras, resolution_scale, args)
            
        
        # process meshes
        if gaussians.binding != None:
            # print('-------', len(scene_info.train_meshes), len(scene_info.test_meshes))
    ...
    def getCanoCameras(self, scale=1.0):
        return CameraDataset(self.cano_cameras[scale])