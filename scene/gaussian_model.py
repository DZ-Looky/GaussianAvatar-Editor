...
class GaussianModel:
    ...
    def capture(self):
        return (
            ...
            self.gs_edit_filter,   
            self.gs_edit_face_filter,
        )

    def restore(self, model_args, training_args, different_data, use_gan=False):
        try:
            (self.active_sh_degree, 
            self._xyz, 
            self._features_dc, 
            ...
            self.spatial_lr_scale,
            self.gs_edit_filter,
            self.gs_edit_face_filter) = model_args
        except:
            (self.active_sh_degree, 
            self._xyz, 
            self._features_dc, 
        ...
        #         print("dddsdsd", d.shape)
        #     else :
        #         print("not kdkdkdkd", d)
        ...
        # if different_data:  # if using differnt data (different data amout) of the load ckpt and the trianing data, do not load the Adam optimizer paramers from the ckpt # lxy
        #     pass
        # else:
        #     if self.use_gan:
        #         assert len(opt_dict) == len(self.optimizer.param_groups) or len(opt_dict) + 1 == len(self.optimizer.params), "ERROR!"
        #         if len(opt_dict) + 1 == len(self.optimizer.params):
        #             opt_dict.append(self.optimizer.params[-1].deepcopy())
        #     self.optimizer.load_state_dict(opt_dict)
        
        try:
            self.optimizer.load_state_dict(opt_dict)
        except:
            print("Not loading optimizer params in the loading ckpt")
            pass
    ...
    def create_from_pcd(self, pcd : Optional[BasicPointCloud], spatial_lr_scale : float):
        ...
        features[:, 3:, 1:] = 0.0
        self._xyz = nn.Parameter(fused_point_cloud.requires_grad_(True))   # [10144, 3]
        self._features_dc = nn.Parameter(features[:,:,0:1].transpose(1, 2).contiguous().requires_grad_(True))    # [10144, 1, 3]
        self._features_rest = nn.Parameter(features[:,:,1:].transpose(1, 2).contiguous().requires_grad_(True))   # [10144, 15, 3]
    ...
    def training_setup(self, training_args):
        ...
        #     name = d["name"]
        #     v = d["params"]
        #     # if 1004 in v[0].shape or 1119 in v[0].shape:
        #     print(" param ",name, ' ', v[0].shape)

        if self.use_gan:
            l.append({'params': self.discriminator.parameters(), 'lr': 1e-4, "name": "discriminator"}) 
    ...
    def prune_points(self, mask):    # mask [87667]
        ...
        optimizable_tensors = self._prune_optimizer(valid_points_mask)   # prune in this func
        ...
        if self.binding != None:
            ...
            if (self.gs_edit_filter is not None) and (len(self.binding) != len(self.gs_edit_filter)):  # lxy
                # update 
                self.gs_edit_filter = self.gs_edit_filter[valid_points_mask.cpu()] 
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        if hasattr(self, "binding"):
            ...
            if (self.gs_edit_filter is not None) and (len(self.binding) != len(self.gs_edit_filter)):  # lxy
                # update 
                new_gs_edit_filter = self.gs_edit_filter[new_binding.cpu()]
                self.gs_edit_filter = torch.cat((self.gs_edit_filter, new_gs_edit_filter))
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        selected_pts_mask = torch.logical_and(selected_pts_mask,
                                              torch.max(self.get_scaling, dim=1).values <= self.percent_dense*scene_extent)
        print("Gaussian AfAdsasdaSdad00000 end number", selected_pts_mask.sum(), " ", selected_pts_mask.shape)
        ...
        if hasattr(self, "binding"):
            # Toyota Motor Europe NV/SA and its affiliated companies retain all intellectual property and proprietary rights in and to the following code lines and related documentation. Any commercial use, reproduction, disclosure or distribution of these code lines and related documentation without an express license agreement from Toyota Motor Europe NV/SA is strictly prohibited.
            new_binding = self.binding[selected_pts_mask]      # new_binding's value is faces_id
            self.binding = torch.cat((self.binding, new_binding))
            self.binding_counter.scatter_add_(0, new_binding, torch.ones_like(new_binding, dtype=torch.int32, device="cuda"))
            
            # if self.gs_edit_filter is not None:  # lxy
            if (self.gs_edit_filter is not None) and (len(self.binding) != len(self.gs_edit_filter)):  # lxy
                # update 
                new_gs_edit_filter = self.gs_edit_filter[new_binding.cpu()]
                self.gs_edit_filter = torch.cat((self.gs_edit_filter, new_gs_edit_filter))
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size):
        ...
        print("Gaussian AfAdsasdaSdad end number", prune_mask.sum(), " ", prune_mask.shape)
        ...
        self.prune_points(prune_mask)  # this step is core
