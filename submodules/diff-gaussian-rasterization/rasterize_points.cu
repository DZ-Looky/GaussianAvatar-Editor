...
std::tuple<int, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor,
           torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>    // need change its 
RasterizeGaussiansCUDA(
  ...
  // torch::Tensor out_gs_render_id = torch::full({10, H, W}, 0.0, float_opts);
  ...
  if (P != 0) {
    ...
    rendered = CudaRasterizer::Rasterizer::forward(
        ...
        // out_gs_render_id.contiguous().data<float>(), 
        out_gs_render_id.contiguous().data<int>(), 
        out_render_alphaT.contiguous().data<float>(),
        radii.contiguous().data<int>(),
        debug);
  }
  return std::make_tuple(rendered, out_color, out_depth, out_gs_render_id, out_render_alphaT, radii, geomBuffer,
